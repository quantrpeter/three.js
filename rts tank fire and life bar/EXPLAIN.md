...
UNIT_MAX_HEALTH = 100, BULLET_DAMAGE = 25 — each unit takes 4 hits to kill.
createHealthBar(unit) — creates a 3D billboard bar (dark background + colored foreground) added to the scene when a unit is placed.
updateHealthBarVisual(unit) — shrinks the bar left-to-right and changes color: green → yellow → red as health drops.
updateHealthBars() — called every frame; positions each bar 0.45 units above the unit (along the planet normal) and rotates it to always face the camera.
damageUnit(unit, amount) — decreases health and calls destroyUnit at 0.
destroyUnit(unit) — triggers an explosion, removes the health bar, clears bullet targets, removes the unit from the scene and placedUnits.
Bullet hit now calls damageUnit(ud.target, BULLET_DAMAGE) instead of just creating an explosion.
